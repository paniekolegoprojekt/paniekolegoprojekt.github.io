export const AboutMeSection = () => {
  return (
    <div className="md:max-w-[1000px] w-full lg:m-auto bg-white" id="aboutme">
      <div className="paragraph-m lg:paragraph-xl text-justify max-w-unset m-8 lg:mb-24 mb-8">
        <p className="text-xl lg:text-4xl my-12 xl:my-32 leading-normal py-8 border-y-2 ">
          Nazywam się <b>Eryk Rozdolski</b> i robię to co należy. To znaczy
          jeśli uważam, że coś należy zrobić i nie ma właściwszej osoby ode
          mnie, to czuję się w obowiązku żeby to zrealizować.
        </p>
        <p className="mt-4">
          Od 2017 roku komercyjnie w<b> Panie Kolego Projekt. </b>
          Profesjonalnie zajmuję się tworzeniem oprogramowania jako
          frontend-developer. Poprzez swoje doświadczenie i zainteresowania
          projektowe, łatwiej jest mi poruszać się w tematach ui/ux i tematów
          związanych z designem.
        </p>
        <p className="mt-4">
          Miałem to szczęście pisać komercyjnie w wielu różnych językach tj.
          Python, OCaml, javascript, co pozwoliło mi nabyć dość szeroki pogląd
          paradygmatyczny, jeśli chodzi o moją intuicję jak powinien wyglądać
          czysty kod. Dziś najczęściej piszę w javascript-cie i poruszam się
          wokół problemów powiązanych z<b> Reactem, next.js</b> czy szeroko
          pojętym web-developmentem.
        </p>
        <p className="">
          Swój wolny czas lubię spędzać w aktywny sposób, długo grałem amatorsko
          w piłkę nożną. Poświęcam sporo czasu innym sportom, ostatnio trochę
          więcej skakance i szachom. Od czasu do czasu próbuję nagrywać własną
          muzykę, a jeśli to się nie udaje to słucham cudzej.
        </p>
      </div>
    </div>
  );
};
